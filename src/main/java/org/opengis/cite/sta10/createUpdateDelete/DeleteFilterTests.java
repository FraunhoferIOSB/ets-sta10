...
import static org.opengis.cite.sta10.filteringExtension.DateTimeTests.getFromListExcept;
import static org.opengis.cite.sta10.filteringExtension.DateTimeTests.removeFromList;
...
public class DeleteFilterTests {
    ...
    @Test(description = "Test deleting observations by time.", groups = "level-3")
    public void testDeleteByTime() throws ServiceFailureException {
        ...
        List<Observation> remaining = getFromListExcept(observations, 0, 1, 21);
        deleteAndCheck(doaDs1, String.format("resultTime lt %s", T700), remaining);
        deleteAndCheck(doaDs1, String.format("validTime lt %s", T700), removeFromList(observations, remaining, 8, 9, 23));
        deleteAndCheck(doaDs1, String.format("%s lt phenomenonTime", T800), removeFromList(observations, remaining, 6, 7, 15, 22, 24));
    }
